...
def run_incremental_training(
    ...
    假设：
    - TrainConfig.libero_task_sequence, TrainConfig.steps_per_task 已经配置好
    - DataConfig.libero_task_indices 用来驱动 LiberoTaskFilteredDataset 的过滤
    """
    ...
    for task_id in config.libero_task_sequence:
...
# 如果两者都有，都打印出来
        if hasattr(config, 'libero_task_indices'):
            print(f"3. config.libero_task_indices: {config.libero_task_indices}")

        if hasattr(config.data, 'libero_task_indices'):
            print(f"4. config.data.libero_task_indices: {config.data.libero_task_indices}")
        # 保存 task_indices

        task_data_config = dataclasses.replace(config.data)

        # 动态添加任务索引字段
        task_data_config = dataclasses.replace(
        config.data,
        libero_task_indices=[task_id]  # 前提：DataConfig 有这个字段
        )
...
def main(config: _config.TrainConfig):
    ...
    # JAX compilation cache（根据你机器路径改）
