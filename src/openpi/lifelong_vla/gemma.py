...
"""Gemma adaptation for the isolated LifelongVLA branch.
...
from openpi.lifelong_vla import dual_lora as lora
...
class Config:
    ...
    num_kv_heads: int
    head_dim: int
    lora_configs: dict[str, lora.DualLoRAConfig] = dataclasses.field(default_factory=dict)


Variant = Literal["dummy", "gemma_300m", "gemma_2b"]


def get_config(variant: Variant | str, adapter: lora.DualLoRAConfig | None = None) -> Config:
    ...
    if variant == "dummy":
        ...
        return dataclasses.replace(config, lora_configs={"attn": adapter, "ffn": adapter}) if adapter else config
...
class Attention(nn.Module):
    ...
    @nn.compact
    def __call__(self, xs, positions, attn_mask, kv_cache, gate_context, gradient_mode):
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            ...
            if config.num_kv_heads == config.num_heads:
                qkv_einsum = lora.Einsum(
                    ...
                    config=config.lora_configs["attn"],
                )
                qkvs.append(qkv_einsum("BSD,3KDH->3BSKH", x, gate_context, gradient_mode))
            else:
                ...
                q = q_einsum("BTD,NDH->BTNH", x, gate_context, gradient_mode)
                ...
                k, v = kv_einsum("BSD,2KDH->2BSKH", x, gate_context, gradient_mode)
                qkvs.append((q, k, v))
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                out.append(out_einsum("BTNH,NHD->BTD", encoded[:, start:end], gate_context, gradient_mode))
                start = end
            else:
                out.append(None)
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        post_attn, kv_cache = attn(pre_attn, positions, attn_mask, kv_cache, gate_context, gradient_mode)
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                x = lora.FeedForward(  # noqa: PLW2901
                    ...
                    name=_name("mlp", i),
                    config=config.lora_configs["ffn"],
                )(x, gate_context, gradient_mode)
            out.append(x)
            gates.append(gate if x is not None else None)
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        gate_context: at.Float[at.Array, "b d"] | None = None,
        gradient_mode: lora.GradientMode = "both",
        *,
        ...
        if gate_context is None:
            gate_context = jnp.zeros((mask.shape[0], self.configs[0].width), dtype=self.embed_dtype)

        embedded, kv_cache = self.layers(
            embedded, kv_cache, positions, mask, adarms_cond, gate_context, gradient_mode, deterministic
        )
    ...
    def init(self, use_adarms: Sequence[bool]):
        ...
        self(
            ...
            gate_context=jnp.zeros((1, self.configs[0].width)),
        )
